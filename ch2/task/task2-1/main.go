...
// Celsius 和 Fahrenheit底层type都为float64
...
func main() {
	arg := os.Args[1:]
	// 只要底层是一样的，那么就可以显示的转换，比如 Fahrenheit和Celsius float64
	// 但是一定要显示的转化，go里 int float可以显示的转化不过会出现丢掉小数点的行为。
	// 并且一定要判断int的最大值，不然会存在一定的bug.
	for _, v := range arg {
